...
/* USER CODE BEGIN PV */
uint8_t data1[LCD_RAM_SIZE];
uint8_t data2[LCD_RAM_SIZE];
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	for(uint16_t j = 0; j < Pixel_NUM; j++) {
		data1[j * 2] = (uint8_t)(0x2fe0 >> 8);
		data1[j * 2 + 1] =  (uint8_t)(0x2fe0);
	}
	for(uint16_t j = 0; j < Pixel_NUM; j++) {
		data2[j * 2] = (uint8_t)(0xffff >> 8);
		data2[j * 2 + 1] =  (uint8_t)(0xffff);
	}
