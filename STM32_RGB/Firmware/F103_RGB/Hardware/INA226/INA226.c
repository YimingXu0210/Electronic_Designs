...
void INA226_init(void)
{
    ...
    INA226_SetConfig(INA226_MODE_CONT_SHUNT_VOLTAGE | INA226_VBUS_8244uS | INA226_AVG_1);

	/*
	* 分流电阻最大电压 = 32768 * 0.0000025V = 0.08192V
	* 设置分流电压转电流转换参数:电阻0.02R，分辨率0.2mA
	* 公式1
	* Current_LSB = 预期最大电流 / 2^15
	* Current_LSB = 10 / 32768 = 0.0003051A ,选0.3ma
	* 公式2
	* CAL = 0.00512/(Current_LSB*R)
	* CAL = 0.00512/(0.0003*0.02)=853 = 0x0355
	*/
//    INA226_SetCalibrationReg(0x0355);

	/*
	* 分流电阻最大电压 = 32768 * 0.0000025V = 0.08192V
	* 设置分流电压转电流转换参数:电阻3.3R，分辨率0.0000008A
	* 公式1
	* Current_LSB = 预期最大电流 / 2^15
	* Current_LSB = 25mA / 32768 = 0.00000076A ,选0.0000008a
	* 公式2
	* CAL = 0.00512/(Current_LSB*R)
	* CAL = 0.00512/(0.0000008*3.3)=1939 = 0x0793
	*/
...
//	IM = 25.f / 1000.f;
    ...
    INA226_SetCalibrationReg((uint16_t)(CAL));
...
float INA226_GetCurrent()
{
	...
	fCurrent = regData * LSB;/*电流的LSB，由用户配置*/
	return fCurrent;
}
/*
**************************************************
...
float INA226_GetPower()
{
	...
	fPower = regData * LSB * 25;/*功率的LSB = 电流的LSB*25*/
	return fPower;
}
